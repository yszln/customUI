package com.yszln.advancedui.utils;
...
public class StatusBarUtil {
     ...
     * @param statusBarAlpha 状态栏透明度
     */
    public static void setColorForSwipeBack(AppCompatActivity activity, @ColorInt int color, @IntRange(from = 0, to = 255) int statusBarAlpha) {
...
//            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            //设置为黑色半透明
//            setTranslucentView((ViewGroup) window.getDecorView(), Color.BLACK, 0.2f);
            //设置状态栏半透明
    ...
    public static void darkMode(Activity activity, boolean dark) {
        if (isFlyme4Later()) {
            ...
            darkModeForMIUI6(activity, activity.getWindow(), dark);
    ...
    public static void darkMode(AppCompatActivity activity) {
        darkMode(activity, activity.getWindow(), DEFAULT_COLOR, DEFAULT_ALPHA);
    }

    public static void darkMode(AppCompatActivity activity, int color, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        darkMode(activity, activity.getWindow(), color, alpha);
    }

    /**
     * 设置状态栏darkMode,字体颜色及icon变黑(目前支持MIUI6以上,Flyme4以上,Android M以上)
     */
    public static void darkMode(AppCompatActivity activity, Window window, int color, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        if (isFlyme4Later()) {
            darkModeForFlyme4(window, true);
            immersive(window, color, alpha);
        } else if (isMIUI6Later()) {
            darkModeForMIUI6(activity, window, true);
            immersive(window, color, alpha);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
     ...
     */
    public static boolean darkModeForMIUI6(Activity activity, Window window, boolean darkmode) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            darkModeForM(activity.getWindow(), darkmode);
        }
    ...
    public static boolean isMIUI6Later() {
        try {
            ...
            //            Log.e("判断是否为MIUI6以上", "---MIUI版本---" + version);
            return version >= 6;
        } catch (Exception e) {
            return false;
        }
     ...
     * @return
     */
    public static int getPaddingSmart(Context context) {
        if (Build.VERSION.SDK_INT >= MIN_API) {
